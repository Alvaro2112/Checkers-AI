...
def Minimax(board, depth,stack):
    ...
    for i in board.black_pieces:
        ...
        for j in i.legal_moves:
            ...
            next_pos = copyr.black_pieces[a].legal_moves[b].to
            copyr.black_pieces[a].move_to(next_pos[0],next_pos[1])

            _, _, score = Minimax(copyr, depth-1,stack)
            move = j
            #board = save
            if score < best_score:
                best_move = move
                ...
                '''
                    ...
                    for j in move.eaten:
                            ...
                            if whos_turn == 1:
                                ...
                                boardx.white_pieces.remove(boardx.get_piece(j.row,j.col))
                  '''
                piece_from, move , _ = Minimax(boardx , 1,stack)
                to = move.to
                piece_from.move_to(to[0],to[1])
